...
def plot_acc(log_name, color="r"):
    ...
    plt.plot(x_train, train_acc, '-', linestyle='--',
             color=color, linewidth=2, label=train_name)
    plt.plot(x_val, val_acc, '-', linestyle='-',
             color=color, linewidth=2, label=val_name)
    plt.legend(loc="best")
...
def main():
    ...
    args.out = log_files[0].replace('.log', '.png')
    if args.out[0:4] != 'log/':
        args.out = os.path.join('log', args.out)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--logs', type=str, default="log/resnet-50.log",
                        help='the path of log file, --logs=resnet-50.log,resnet-101.log')
    parser.add_argument('--out', type=str, default="log/training-curve.png",
                        help='the name of output curve ')
